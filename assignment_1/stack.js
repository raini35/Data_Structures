...
*
*                      COMBINE FUNCTION 
*	Normalizing as in combining all like 
************************************************************************/
function combine_same_degrees_in(stack) {
...
function multiplication(right_operand, left_operand) {
	...
	return_stack = combine_same_degrees_in(return_stack); 
...
function perform_calculation(stack_input, operators) {
	...
	//Goes through the operators list and performs the calculations 
...
function output_stack(stack_input, operators, solution) {
	var string= ""; 
	var iterator = 0; 
	//Prints out the polynomials
	for(var i = 0; i < stack_input.length; i++) {
		for(var h = 1; h <= stack_input[i]._size; h++) {
		
			if(stack_input[i]._degree[h] == '0') {
				string = string + stack_input[i]._coef[h]; 

			}
			else if (stack_input[i]._degree[h] == '1') {
				string = string + stack_input[i]._coef[h] + "x"; 
	...
	//Creates an array of all the keys in the hash 
	for (var key in solution) {
		reverse_key.push(key); 
	}
	
	//I traverse the hash in reverse so I can order it by descending order
	for (var m=reverse_key.length-1; m>=0; m--) {
		var key = reverse_key[m]; 
